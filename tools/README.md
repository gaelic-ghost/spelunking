...
- `mediaremote-xpc-trace-observe.zsh`: builds `MRXPCTraceInterpose`, injects it into a probe with `DYLD_INSERT_LIBRARIES`, and runs the command through `mediaremote-daemon-observe.zsh` so XPC message-ID traces and daemon logs share one capture window.
